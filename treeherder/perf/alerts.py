...
# TODO: DRY this
def simulate_alert_generation(signature: PerformanceSignature) -> None:
    # get series data starting from either:
    # (1) the last alert, if there is one
    # (2) the alerts max age
    # (use whichever is newer)
    logger.info(
        f'Simulating alert generation for signature ID {signature.id}'
        f'({signature.repository.name} repository, {signature.framework.name} framework)...'
    )
    max_alert_age = datetime.now() - settings.PERFHERDER_ALERTS_MAX_AGE
    series = PerformanceDatum.objects.filter(signature=signature, push_timestamp__gte=max_alert_age)
    latest_alert_timestamp = (
    ...
    for data_point in series:
        if not revision_data.get(data_point.push.revision):
            revision_data[data_point.push.revision] = RevisionDatum(
                int(time.mktime(data_point.push_timestamp.timetuple())),
                data_point.push_id,
                [],
                str(data_point.push_timestamp),
            )
        revision_data[data_point.push.revision].values.append(data_point.value)
    from pprint import pprint

    pprint(revision_data)
    ...
    for (prev, cur) in zip(analyzed_series, analyzed_series[1:]):
        if cur.change_detected:
            logger.info('Some change is noticeable')
            prev_value = cur.historical_stats['avg']
            new_value = cur.forward_stats['avg']
            alert_properties = get_alert_properties(
                prev_value, new_value, signature.lower_is_better
            ...
            push = Push.objects.get(cur.push_id)
            prev_push = Push.objects.get(cur.prev_push_id)
            logger.info(
                f'Detected performance change on suspect range '
                f'{push.revision}..{prev_push.revision}'
            )
            # summary, _ = PerformanceAlertSummary.objects.get_or_create(
            #     repository=signature.repository,
            #     framework=signature.framework,
            ...
            if t_value == float('inf'):
                t_value = 1000
            logger.info(f'Creating alert {alert_properties}')
            # PerformanceAlert.objects.update_or_create(
            #     summary=summary,
            #     series_signature=signature,
            #     defaults={
    ...
    if no_change_detected:
        logger.info('No performance change detected')
